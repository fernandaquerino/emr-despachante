 ...
 * suas regras: `tabular-nums`, formato PT-BR, e nunca colorir o valor por
 * semântica de status (checklist de DOMAIN_COMPONENTS.md).
 */
      ...
      <ul className="flex flex-col gap-1.5">
        {lines.map((line) => (
          <li key={line.label} className="flex justify-between text-body-sm text-text-secondary">
            ...
            <span className="tabular-nums">{formatCentsToBRL(line.amountCents)}</span>
          </li>
        ))}
      </ul>
      <div className="flex justify-between border-t border-border pt-2 text-body font-semibold text-text">
        <span>{totalLabel}</span>
        <span className="text-kpi tabular-nums">{formatCentsToBRL(totalCents)}</span>
      </div>
    </div>
  );
}

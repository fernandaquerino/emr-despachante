...
- scripts antigos `US-*`, quando existirem no histórico do repositório

## Cobertura por tela

Para implementação frontend, a fonte preferida é `create-emr-backlog.sh`, que usa issues `FE-*`. Referências `US-*` antigas ficam como histórico/backlog base quando ainda existirem.
...
| Solicitações do Admin | `US-PRT-006`, `US-PRT-007` | Coberta |
...
| Usuários internos | `US-PT-008`, `US-903` | Coberta |
| Detalhe do usuário interno | `US-PT-008`, `US-903` | Coberta |
...
- Explicitei tarefas para `/admin/solicitacoes` e `/admin/solicitacoes/:id`.
...
- O MVP atual usa OWNER, PARTNER e ADMIN. Operação interna pertence ao ADMIN.
- As issues `FE-OPS-*` continuam válidas como agrupamento de operação, mas suas rotas são `/admin/*`.
